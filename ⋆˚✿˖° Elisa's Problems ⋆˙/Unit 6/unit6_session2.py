...
top_hits_2010s = SongNode("Uptown Funk", node_two)
...
Evaluate the time complexity of your solution. Define your variables and provide a rationale for why you believe your 
...
def get_artist_frequency(playlist):
    ...
    while current:
        if current.artist in playlist_mapping:
            playlist_mapping[current.artist] +=1
        else: 
            playlist_mapping[current.artist] = 1 
        current = current.next #--------------- make sure that this is in the while loop 
...
#U: 
#input: head of llinked list playlist, 
#output: dictionary mapping frequencies + artisits
#constraints: (limitations) time/space complexity
#edge cases: if artist isnt in dictionary yet

#M:
#linked lists, dictionaries(mapping), 

#P:

'''init dict

set current to playlist(head)

while current: traverse through the linked list  --> as long as it exists 
    check if the artist is in the dict 
        update frequency +=1


    else if: define its own value entry 
    set to 1 (for first occurance)

    update to the next current 
    ^^ current = current.next
